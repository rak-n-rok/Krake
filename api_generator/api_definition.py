...
.. code:: bash
    ...
    python -m api_generator api_definition <module_path> [[other_opts]...] > \
        <module.path>
...
class ResourceDef(NamedTuple):
    """Represents the API definition of a Krake resource, as it will be added to the
    ...
    @property
    def namespaced(self):
        """Computes if a resource is namespaced or not.
    ...
    def from_resource(
        ...
        """Creates a :class:`ResourceDef` from a resource and its API name.
...
def is_persistent_class(obj):
    """Checks if an object is a class that is persisted in the Krake database.
...
class Api(NamedTuple):
    """Represents an API object that will be put into the template.
    ...
    def create_resource_definitions(cls, api_name, data_classes, scopes):
        """For each persisted class in the API, creates a corresponding
    ...
    def create_import_classes(cls, resource_definitions):
        """Creates the list of classes that need to be imported for the API definition
    ...
    def from_path(cls, module_path, name=None, scopes=None):
        """Creates an instance of :class:`Api` from the data structures of a Krake API.
...
def process_classes_scopes(scopes, data_classes):
    """Parses all provided scopes to check the given class and scope values.
...
def add_apidef_subparser(subparsers):
    """Adds the subparser that is specific for the generation of the API definition, as
    ...
    parser.add_argument(
        ...
        dest="scopes",
        help=(
            "Specifies the scope that should be used for an individual class of the "
