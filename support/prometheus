...
async def heat_demand_metric(zone, interval):
...
async def start_metrics_tasks(zones, interval, app):
    app["metrics"] = [
        app.loop.create_task(heat_demand_metric(zone, interval))
...
async def exporter(zones, interval, port):
    exporter_port = port + 1

    app = web.Application()
    app.router.add_get("/metrics", aio.web.server_stats)
    app.on_startup.append(functools.partial(start_metrics_tasks, zones, interval))
...
if __name__ == "__main__":
    ...
    loop.create_task(exporter(args.zones, args.interval, args.port))
