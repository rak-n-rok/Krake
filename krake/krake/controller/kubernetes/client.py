...
class KubernetesClient(object):
    ...
    * prepare the connection from the information stored in the cluster's kubeconfig
      file;
    * prepare the connection to a custom resource's API if a Kubernetes resource to
      manage relies on a Kubernetes custom resource;
    * offers two methods, :meth:`apply` and :meth:`delete` respectively to properly
      apply a manifest (create or update a resource), or properly delete a resource.
    ...
    Attributes:
        ...
        custom_resources (list[str]): name of all custom resources that are available on
            the current cluster.
        resource_apis (dict): mapping of a Kubernetes's resource name to its internal
            API object.
