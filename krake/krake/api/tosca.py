...
# Set `toscaparser` logger to the ERROR level.
# The default INFO level is quite verbose.
...
class ToscaParser(object):
    ...
    Attributes:
        ...
        path (str): TOSCA template path or CSAR path.

    """

    def __init__(self, yaml_dict_tpl=None, path=None):
        assert (
            yaml_dict_tpl or path
        ), "TOSCA should be defined by python dict or by file"
        self.tosca_template = self._get_tosca_template(yaml_dict_tpl, path)
    ...
    @staticmethod
    def _get_tosca_template(yaml_dict_tpl=None, path=None):
        ...
        if path:
            try:
                return ToscaTemplate(path=path)
            except ValidationError as err:
                raise ToscaParserException(err, formatter=fmt_validation_error)
