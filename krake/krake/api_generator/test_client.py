...
client of the provided API definition, given as the Python path to its Krake data
module.
...
.. code:: bash
    ...
    python -m krake.api_generator test_client <module_path> [<other_opts>...] > \
        <module.path>
...
.. code:: bash
    ...
    python -m krake.api_generator test_client krake.apidefs.core
...
def generate_test_client(
    ...
    code of an implementation of the unit tests for the client for this API definition,
    to be included in Krake.
...
def add_apidef_subparser(subparsers):
    ...
    API clients, as well as its arguments.
    ...
    parser = subparsers.add_parser(
        "test_client",
        help=(
            "Generation of the unit tests for the API client,"
    ...
    default = os.path.join(get_default_template_dir(), "test_client")
    add_templates_dir(parser=parser, default=default)

    add_no_black_formatting(parser=parser)

    parser.set_defaults(generator=generate_test_client)
