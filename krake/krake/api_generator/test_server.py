"""This specific module handles the generation of the code for the unit tests of the API
server code of the provided API definition, given as the Python path to its Krake data
module.

The generator prints a file content. This should be put into a Python file, which can
then be integrated into Krake, in the unit tests.

The following syntax should be used:

.. code:: bash

    python -m krake.api_generator test_server <module_path> [<other_opts>...] > \
        <module.path>
...
.. code:: bash
    ...
    python -m krake.api_generator test_server krake.apidefs.core
...
def generate_test_server(
    data_path, templates_dir, no_black, template_path="main.jinja"
):
    """From a given Krake API definition path, print a Python file that contains the
    code of an implementation of the unit tests for the server code of this API
    definition, to be included in Krake.
...
def add_apidef_subparser(subparsers):
    """Add the subparser that is specific for the generation of the unit tests for the
    API server code, as well as its arguments.
    ...
    parser = subparsers.add_parser(
        "test_server",
        help=(
            "Generation of the unit tests for the API server code,"
            "using an API definition as base.",
        ),
    )
    ...
    default = os.path.join(get_default_template_dir(), "test_server")
    add_templates_dir(parser=parser, default=default)

    add_no_black_formatting(parser=parser)

    parser.set_defaults(generator=generate_test_server)
