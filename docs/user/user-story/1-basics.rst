...
-- **Alternatively**, spawn a Kubernetes ``Application`` using its ``TOSCA`` template file or ``CSAR`` archive, see :ref:`dev/tosca:Examples`.
...
-- **Alternatively**, update a ``TOSCA`` template file or ``CSAR`` archive to create a second Pod for the ``echo-demo`` application, see :ref:`dev/tosca:Examples`.
