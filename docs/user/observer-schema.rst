...
Kubernetes Observer. But what parts of the Kubernetes resource should be "observed" by
Krake ? The purpose of the Observer Schema is to provide a flexible mean to the Krake
users to define which fields of the Kubernetes resources should be "observed" and which
shouldn't.
...
In the example above, the user didn't specify a custom manifest file for the
...
In order to identify which resource a schema is refering to, the ``apiVersion``,
``kind`` and ``name`` need to be specified. Those fields are also the minimum fields a
user can specificy in order to observe a resource. As a result, and without additional
...
comprised between 1 and 4. If the length of the ``ports`` list is below 1 or above 4,
Krake reverts the ``Service`` to its last known value.
